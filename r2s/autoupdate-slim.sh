...
wget https://github.com/ardanzhu/Opwrt_Actions/releases/download/$(date +%Y-%m-%d)-slim/FriendlyWrt_$(date +%Y%m%d)_NanoPi-R2S_arm64_sd.img.gz
if [ -f /mnt/mmcblk0p2/Friend*.img.gz ]; then
	echo -e '\e[92m今天固件已下载，准备解压\e[0m'
else
	echo -e '\e[91m今天的固件还没更新，尝试下载昨天的固件\e[0m'
	wget https://github.com/ardanzhu/Opwrt_Actions/releases/download/$(date +%Y-%m-%d)-slim/FriendlyWrt_$(date -d "@$(( $(busybox date +%s) - 86400))" +%Y%m%d)_NanoPi-R2S_arm64_sd.img.gz
	if [ -f /mnt/mmcblk0p2/Friend*.img.gz ]; then
