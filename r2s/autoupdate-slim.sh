...
wget https://github.com/ardanzhu/Opwrt_Actions/releases/download/$(date +%Y-%m-%d)-slim/R2S-slim-$(date +%Y-%m-%d).zip
if [ -f /mnt/mmcblk0p2/R2S*.zip ]; then
	echo -e '\e[92m今天固件已下载，准备解压\e[0m'
else
	echo -e '\e[91m今天的固件还没更新，尝试下载昨天的固件\e[0m'
	wget https://github.com/ardanzhu/Opwrt_Actions/releases/download/$(date -d "@$(( $(busybox date +%s) - 86400))" +%Y-%m-%d)-slim/R2S-slim-$(date -d "@$(( $(busybox date +%s) - 86400))" +%Y-%m-%d).zip
	if [ -f /mnt/mmcblk0p2/R2S*.zip ]; then
